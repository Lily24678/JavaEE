            ...
            <div class="row" id="pidHistory">
                <c:if test="${cookie.pidHistory.value!=''}">
                    <c:forTokens items="${cookie.pidHistory.value}" delims="-" var="pid">
                        <sql:query dataSource="${snapshot}" var="phistory">
                            select * from product where pid=?;
                            <sql:param value="${pid}"/>
                        </sql:query>
                        <div class="col-md-1" style="text-align: center; height: 100px;">
                            <a href="${pageContext.request.contextPath}/demo/jsp/product.jsp?${phistory.rows[0].pid}"><img
                                    alt="图片无法显示"
                                    src="${pageContext.request.contextPath}/img/products/${phistory.rows[0].imgurl}"
                                    width="100%"
                                    height="130"></a>
                        </div>
                    </c:forTokens>
