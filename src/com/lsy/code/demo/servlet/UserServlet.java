...
public class UserServlet extends BaseServlet {
	...
	public String regist(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException {
		...
		User user = q.insert(sql, new BeanHandler<User>(User.class), username,password);
	...
	public void checkUserLogin(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (StringUtils.isBlank(username) || StringUtils.isBlank(password)) {
			 ...
			 * else if (CreateDataUtils.isExist(username, password) == 0) { massage =
			 * MessageHandler.createMsgFailure("用户名或者登录密码错误"); }
			 */ else {
